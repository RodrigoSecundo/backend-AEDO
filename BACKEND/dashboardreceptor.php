...
if (!isset($_SESSION['perfil']) || $_SESSION['perfil'] != 'receptor') {
    // Redirecionar para a página de login se não estiver logado ou não for receptor
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['nome'], $_POST['telefone'], $_POST['id_doador'], $_POST['endereco'], $_POST['rua'], $_POST['complemento'], $_POST['cep'], $_POST['tipo_convenio'])) {
        ...
        $sql = "UPDATE receptor_info SET 
                ...
                WHERE user_id = :user_id"; // Garantindo que apenas o receptor logado possa atualizar seus dados
    ...
    if (isset($_POST['orgao'])) {
        ...
        $sql = "UPDATE orgaos_necessitados SET orgao = :orgao WHERE user_id = :user_id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':orgao' => $orgao,
            ':user_id' => $_SESSION['user_id']
